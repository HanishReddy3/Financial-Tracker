...
const [formData, setFormData] = useState({
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      try {
        const response = await fetch('http://localhost:8080/expense/all', {
...
try {
      ...
      if (response.ok) {
        ...
        setFormData({ title: '',description: '', category: '', amount: '' }); // Reset form
      } else {
  ...
  return (
    <div style={styles.page}>
      <div style={styles.inner}>
        <h1 style={styles.heading}>💸 Your Expenses</h1>
